...
// Shared with the admin panel's "log a call" form, so a lead the office types
// in lands in the same district, with the same borough and the same in/out of
// area verdict, as one sent through this form.
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  ...
  const { error } = await supabase.from('enquiries').insert({
    ...
    consent: true,
    // Explicit, though the column defaults to it. This is the one place in the
    // codebase entitled to write 'form', and the conversion rates on
    // /admin/activity count exactly the rows that carry it.
    channel: 'form',
